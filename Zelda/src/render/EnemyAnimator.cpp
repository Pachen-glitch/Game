...
void EnemyAnimator::setupSlime() {
    auto reg = [this](const std::string& name, const std::string& state, float dur, bool loop) {
        auto frames = AssetPaths::getEnemyAnimFrames("slime", state);
        Animation clip = AssetPaths::buildAnimation(frames, dur, loop);
...
std::string EnemyAnimator::pickClip(const Enemy& enemy) const {
    ...
    if (enemy.isHurtAnimating() || enemy.getAIState() == EnemyState::Hurt) {
        return "hurt";
    }

    EnemyState st = enemy.getAIState();
    if (st == EnemyState::Chase || st == EnemyState::Wander ||
        st == EnemyState::ReturnToSpawn) {
        sf::Vector2f vel = enemy.getVelocity();
        float speed = std::sqrt(vel.x * vel.x + vel.y * vel.y);
        if (speed > 6.f) return "walk";
    }
...
void EnemyAnimator::applyToEntity(Entity& entity, float scale) {
    ...
    if (enemy && CombatFeel::instance().isEnemyFlashing() &&
        (enemy->isHurtAnimating() || enemy->getAIState() == EnemyState::Hurt)) {
